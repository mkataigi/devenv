---
name: git-sync
description: 指定したディレクトリ配下のGitリポジトリを一括で同期・クリーンアップします。複数のリポジトリのメインブランチを最新化し、マージ済みブランチを削除したい場合に使用してください。スペース区切りで複数ディレクトリを指定可能です。
disable-model-invocation: true
user-invocable: true
allowed-tools: Bash
argument-hint: <directory_path> [directory_path2 ...]
---

# Git Sync Skill

ユーザーが指定したディレクトリに対して、以下の処理を順番に実行してください。
引数はスペース区切りで複数のディレクトリパスを受け取れます。複数指定された場合は、各ディレクトリに対して順番に Step 1〜3 の処理を実施し、最後に Step 4 で全ディレクトリの結果をまとめて報告してください。
...
複数ディレクトリを処理した場合は、ディレクトリごとにセクションを分けて報告してください。
